...
public class DiscountRepositoryTest extends AbstractRepositoryTest {
        ...
        void shouldReturnEntityWhenFindingByTitle() {
                ...
                assertThat(discount).isPresent().hasValueSatisfying(d -> {
                        ...
                        assertThat(d.getExpDate()).isAfterOrEqualTo(time.plusHours(2));
                                ...
                                .withFailMessage(
                                                "Expected not null discount with title \"Flash Sales Discount\" but null");
        }
        ...
        void shouldReturnPaging_whenFindingByTitleAndExpDateBetween() {
                ...
                // When
                Page<Discount> page = discountRepository.findByTitleAndExpDateBetween("", beforeTime, afterTime,
                                pageable);
                // Then
        ...
        @Test
        @DisplayName("Should return updated product when updating discount FK by product ID")
        void shouldReturnUpdatedProduct_whenUpdatingDiscountFKByProductId() {
                // Given
                UUID newDiscountId = discountRepository.findEntityByTitle("Daily Sales Discount").get().getId();
                Long productId = productRepository.findByName("T-Shirt").get().getId();
                // When
                productRepository.updateDiscountByProductId(productId, newDiscountId);
                // Then
                assertThat(productRepository.findByName("T-Shirt")).isPresent().hasValueSatisfying(p -> {
                        assertThat(p.getName()).isEqualTo("T-Shirt");
                        assertThat(p.getDiscount().getId())
                                        .isNotNull()
                                        .isEqualTo(newDiscountId);
                });
        }
